...
class UserController extends AbstractController
{
    ...
    public function getCurrentAppUser(Security $security)
    {
        if (!$this->_current_app_user) {
            $this->_current_app_user = $security->getUser();
        }
    ...
    public function quickNew(Request $request, MailerInterface $mailer, Security $security, EntityManagerInterface $em, EventDispatcherInterface $dispatcher)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $ab->setRegistrar($this->getCurrentAppUser($security));

            $em->persist($ab);
            $em->flush();

            $dispatcher->dispatch(new AnonymousBeneficiaryCreatedEvent($ab), AnonymousBeneficiaryCreatedEvent::NAME);
    ...
    public function removeRole(Request $request, User $user, $role, Security $security, EntityManagerInterface $em)
    {
        $session = $request->getSession();
        $current_user = $this->getCurrentAppUser($security);

        // cannot remove a nonexistant role
        if (!$user->hasRole($role)) {
            $session->getFlashBag()->add('warning', $user . ' ne possède pas le rôle ' . $role);
            return $this->redirectToShow($user, $security);
        }
        // only ROLE_SUPER_ADMIN can remove ROLE_ADMIN to users
        if ($role == 'ROLE_ADMIN' && !$security->isGranted('ROLE_SUPER_ADMIN')) {
    ...
    #[IsGranted('ROLE_ADMIN')]
    public function addRole(Request $request, User $user, $role, Security $security, EntityManagerInterface $em)
    {
        $session = $request->getSession();
        $current_user = $this->getCurrentAppUser($security);
    ...
    #[IsGranted('ROLE_USER')]
    public function selfRegistration(Request $request, Security $security, MembershipService $membershipService)
    {
        $session = $request->getSession();
        $membership = $this->getCurrentAppUser($security)->getBeneficiary()->getMembership();
    ...
    public function removeClientUser(Request $request, User $user, $client_id, Security $security, EntityManagerInterface $em)
    {
        $session = $request->getSession();
        if (!$security->isGranted('IS_AUTHENTICATED_FULLY')) {
            throw $this->createAccessDeniedException();
        }
        if (!$security->isGranted('ROLE_ADMIN') && ($this->getCurrentAppUser($security) != $user)) {
            throw $this->createAccessDeniedException();
        }
        if ($client_id) {
    ...
    #[IsGranted('ROLE_USER_VIEWER')]
    public function preUsers(EntityManagerInterface $em)
    {
        /** @var AnonymousBeneficiary[] $anonymousBeneficiaries */
        $anonymousBeneficiaries = $em->getRepository(AnonymousBeneficiary::class)->findBy(
            [],
            ['createdAt' => 'DESC']
        );
    ...
    #[IsGranted('ROLE_USER_VIEWER')]
    public function quickNewRecall(Request $request, AnonymousBeneficiary $anonymousBeneficiary, EventDispatcherInterface $dispatcher, EntityManagerInterface $em)
    {
        $dispatcher->dispatch(new AnonymousBeneficiaryRecallEvent($anonymousBeneficiary), AnonymousBeneficiaryRecallEvent::NAME);
    ...
    public function preUsersDelete(AnonymousBeneficiary $anonymousBeneficiary, SessionInterface $session, EntityManagerInterface $em)
    {
        $em->remove($anonymousBeneficiary);
        $em->flush();
    ...
    private function redirectToShow(User $user, Security $security)
    {
        if ($user->getBeneficiary()) {
            ...
            if ($security->isGranted('ROLE_ADMIN'))
                ...
                return $this->redirectToRoute('member_show', array('member_number' => $memberNumber, 'token' => $user->getTmpToken($session->get('token_key') . $this->getCurrentAppUser($security)->getUsername())));
        } else {
            return $this->redirectToRoute("homepage");
        }
    }
