...
class ShiftGenerateCommand extends ContainerAwareCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $periodRepository = $em->getRepository('App\Entity\Period');
        $admin = $em->getRepository('App\Entity\User')->findSuperAdminAccount();
        ...
        foreach ( $period as $date ) {
            ...
            foreach ($periods as $period) {
                ...
                foreach ($period->getPositions() as $position) {
                    ...
                    $already_generated = $em->getRepository('App\Entity\Shift')->findBy(array('start' => $start, 'end' => $end, 'job' => $period->getJob(), 'position' => $position));
                    if (!$already_generated) {
                        $lastStart = $this->lastCycleDate($start);
                        $lastEnd = $this->lastCycleDate($end);
                        $last_cycle_shift = $em->getRepository('App\Entity\Shift')->findOneBy(array('start' => $lastStart, 'end' => $lastEnd, 'job' => $period->getJob(), 'position' => $position));
