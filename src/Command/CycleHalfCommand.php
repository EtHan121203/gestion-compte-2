...
class CycleHalfCommand extends ContainerAwareCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $members_with_half_cycle = $em->getRepository('App\Entity\Membership')->findWithHalfCyclePast($date, $cycle_type);
        $count = 0;
        foreach ($members_with_half_cycle as $member) {
            ...
            $currentCycleShifts = $em->getRepository('App\Entity\Shift')->findShiftsForMembership($member, $current_cycle_start, $current_cycle_end);
