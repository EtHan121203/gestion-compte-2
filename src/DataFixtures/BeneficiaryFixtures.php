...
class BeneficiaryFixtures extends Fixture implements OrderedFixtureInterface
{
    public function load(ObjectManager $manager): void
    {
        ...
        for ($i = 1; $i <= $beneficiaryCount; $i++) {
            ...
            // Set Flying
            $beneficiary->setFlying((bool)rand(0, 1));
            ...
            // Set User
            if ($i == $roleGoesToId["ROLE_SUPER_ADMIN"]) {
                $user = $this->getReference('superadmin', User::class);
            } else if (in_array($i, (array)$roleGoesToId["ROLE_ADMIN"])) {
                $user = $this->getReference('admin_' . ($i - 50), User::class);
            } else {
