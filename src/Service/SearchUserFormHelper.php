...
class SearchUserFormHelper {
    ...
    public function getSearchForm($formBuilder, $type = null, $disabledFields = []) {
        $formBuilder->add('withdrawn', ChoiceType::class, [
            'label' => $this->parameterBag->get('member_withdrawn_icon') . ' fermé',
            'required' => false,
        ...
        if (!$type) {
            $formBuilder->add('enabled', ChoiceType::class, [
                'label' => $this->parameterBag->get('user_account_enabled_icon') . ' activé',
                'required' => false,
                'choices' => [
                    'activé' => 2,
                    'Non activé' => 1,
        ...
        $formBuilder->add('frozen', ChoiceType::class, [
            'label' => $this->parameterBag->get('member_frozen_icon') . ' gelé',
            'required' => false,
            'choices' => [
                'gelé' => 2,
                'Non gelé' => 1,
        ...
        if (!$type) {
            $formBuilder->add('exempted', ChoiceType::class, [
                'label' => $this->parameterBag->get('member_exempted_icon') . ' exempté',
                'required' => false,
                'choices' => [
                    'exempté' => 2,
                    'Non exempté' => 1,
        ...
        $formBuilder->add('registration', ChoiceType::class, [
            'label' => $this->parameterBag->get('member_registration_missing_icon') . ' adhéré',
            'required' => false,
    ...
    public function processSearchFormAmbassadorData($form, &$qb) {
        ...
        if (!is_null($form->get('compteurlt')->getData())) {
            ...
            $qb = $qb->andWhere('b.membership IN (SELECT IDENTITY(t1.membership) FROM App\Entity\TimeLog t1 WHERE t1.type != 20 GROUP BY t1.membership HAVING SUM(t1.time) < :compteurlt * 60)')
                ->setParameter('compteurlt', $compteurlt);
        }
        if (!is_null($form->get('compteurgt')->getData())) {
            $compteurgt = $form->get('compteurgt')->getData();
            $qb = $qb->andWhere('b.membership IN (SELECT IDENTITY(t2.membership) FROM App\Entity\TimeLog t2 WHERE t2.type != 20 GROUP BY t2.membership HAVING SUM(t2.time) > :compteurgt * 60)')
