...
class ShiftService
{
    ...
    public function __construct(
        ...
        #[Autowire(param: 'due_duration_by_cycle')]
        $due_duration_by_cycle, 
        #[Autowire(param: 'min_shift_duration')]
        $min_shift_duration, 
        #[Autowire(param: 'new_users_start_as_beginner')]
        $newUserStartAsBeginner, 
        #[Autowire(param: 'allow_extra_shifts')]
        $allowExtraShifts, 
        #[Autowire(param: 'max_time_in_advance_to_book_extra_shifts')]
        $maxTimeInAdvanceToBookExtraShifts, 
        #[Autowire(param: 'forbid_shift_overlap_time')]
        $forbidShiftOverlapTime,
        #[Autowire(param: 'use_fly_and_fixed')]
        $use_fly_and_fixed, 
        #[Autowire(param: 'fly_and_fixed_allow_fixed_shift_free')]
        $fly_and_fixed_allow_fixed_shift_free,
        #[Autowire(param: 'use_time_log_saving')]
        $use_time_log_saving, 
        #[Autowire(param: 'time_log_saving_shift_free_min_time_in_advance_days')]
    ...
    {
        $shifts = $this->em->getRepository('App\Entity\Shift')->findAlreadyBookedShiftsOfBucket($shift);
    ...
    public function getShiftBucketFromShift(Shift $shift)
    {
        ...
        $shifts = $this->em->getRepository('App\Entity\Shift')->findBy([
            'job' => $shift->getJob(),
    ...
    {
        return !$this->em->getRepository('App\Entity\Shift')->findShiftsForBeneficiary($beneficiary,
                $start_after,
                null,
                $start_before,
                $end_after)->isEmpty();
